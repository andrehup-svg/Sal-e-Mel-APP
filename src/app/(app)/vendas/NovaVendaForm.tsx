...
type ItemForm = { itemId: number; produtoId: string; quantidade: number };
...
export default function NovaVendaForm({
  ...
  const [sinalManual, setSinalManual] = useState(false);
  ...
  function valorDoItem(item: ItemForm) {
    ...
    if (!cat) return 0;
    return calcularValorItem(cat, item.quantidade);
  }
  ...
  function resetarForm() {
    ...
    setDataEntrega("");
    setSinal(0);
    setSinalManual(false);
    setErro(null);
    setComprovante(null);
  }

  function adicionarItem() {
    setItens((prev) => [...prev, { itemId: proximoItemId++, produtoId: "", quantidade: 25 }]);
  }
  ...
  return (
    <>
      <Button onClick={abrir}>Nova venda</Button>

      <Modal open={aberto} onClose={fechar} widthClass="max-w-lg" closeOnBackdropClick={false}>
        {etapa === "form" ? (
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h3 className="font-display text-xl text-tinta">Nova venda</h3>
              <button
                type="button"
                onClick={fechar}
                aria-label="Fechar"
            ...
            <div className="flex flex-col gap-3">
              ...
              {itens.map((item) => (
                <ItemRow
                  ...
                  onMudarProduto={(produtoId) => atualizarItem(item.itemId, { produtoId })}
